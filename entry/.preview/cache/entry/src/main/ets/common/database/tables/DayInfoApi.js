...
class DayInfoApi {
    ...
    updateData(dayInfo, callback) {
        const valueBucket = generateBucket(dayInfo);
        let predicates = new dataRdb.RdbPredicates(Const.DAY_INFO.tableName ? Const.DAY_INFO.tableName : '');
...
function generateBucket(dayInfo) {
    var _a;
    let valueBucket = {};
    (_a = Const.DAY_INFO.columns) === null || _a === void 0 ? void 0 : _a.forEach((item) => {
        switch (item) {
            case 'date':
